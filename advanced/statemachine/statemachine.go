...
func (m *Machine) Transition(event string) string {
	...
	if next != "" {
		callFunctions(m.Subscribers, current, next)
		...
		if transitions[event].Actions != nil {
			callFunctions(transitions[event].Actions, current, next)
		}
...
func callFunctions(functions []func(string, string), current, next string) {
